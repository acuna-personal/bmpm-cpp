...
#ifndef DMSOUNDEX_CPP
#define DMSOUNDEX_CPP
...
std::string soundx_name(std::string MyStrArg)
{
    ...
    for (i = 0; i < count(MyStrArray); i++) {
        if (strlen(MyStrArray[i]) > 0) {
            // ignore null at ends of array (due to leading or trailing space)
            if (i != 0) {
...
std::string dmsoundex2(std::string MyStrArg)
{
    ...
    while (strlen(MyStr3) > 0) {
        ...
        if (DMDEBUG) {
            printf("pt1 MyStr='%s', MyStr2='%s', MyStr3='%s'\n", MyStr, MyStr2, MyStr3);
        }
        if (i == LenMyStr3) {
            MyStr3 = "";
            // finished
        ...
        if (!allblank) {
            if (DMDEBUG) {
                printf("pt2 MyStr='%s', MyStr2='%s', MyStr3='%s'\n", MyStr, MyStr2, MyStr3);
            }
            dim_dm2 = 1;
            dm2 = {};
            dm2[0] = "";
            ...
            while (strlen(MyStr) > 0) {
                for (rule = 0; rule < count(newrules); rule++) {
                    // loop through the rules
                    if (substr(MyStr, 0, strlen(newrules[rule][0])) == newrules[rule][0]) {
                        // match found
                        //check for $xnewrules branch
                        xr = "!" + newrules[rule][0] + "!";
                        if (strpos(xnewruleslist, xr) != /* ORIG: !== */ false) {
                        ...
                        if (first == 1) {
                            ...
                            if (strlen(MyStr) > 0 && strpos(vowels, MyStr[0]) != /* ORIG: !== */ false) {
                                if (DMDEBUG) {
                                    printf("pt2b vowel: '%s'\n", MyStr[0]);
                                }
                                ...
                                if (DMDEBUG) {
                                    printf("pt2c vowel: '%s'  done\n", MyStr[0]);
                                }
                                // 1st position not a vowel
                            } else {
                                for (ii = 0; ii < dmnumber; ii++) {
                                    if (DMDEBUG) {
                                        std::cout << "pt2d lastdm:\n";
                                    }
                                    if (DMDEBUG) {
                                        print_r(lastdm);
                                    }
            ...
            for (ii = 0; ii < dim_dm2; ii++) {
                ...
                if (ii == 0 && strpos(dm, dm2[ii]) == /* ORIG: === */ false && strpos(dm3, dm2[ii]) == /* ORIG: === */ false) {
                    dm = dm2[ii];
                } else {
                    if (strpos(dm, dm2[ii]) == /* ORIG: === */ false && strpos(dm3, dm2[ii]) == /* ORIG: === */ false) {
                        if (strlen(dm) > 0) {
            ...
            if (DMDEBUG) {
                std::cout << "pt3 - dm3 '" + dm3 + "' dm '" + dm + "'\n";
            }
            if (strlen(dm3) > 0 && strlen(dm) > 0 && strpos(dm3, dm) == /* ORIG: === */ false) {
