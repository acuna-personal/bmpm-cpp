...
php_type Phonetic_UTF8(php_type input, php_type type, php_type allLanguagesBitmap, php_type languageRules, php_type rules, php_type finalRules1, php_type finalRules2, std::string languageArg, bool concat)
{
    ...
    // treat dash as if it were a space
    input = trim(input);
    // remove any leading spaces introduced by above replacement of dashes to spaces
    /*
    echo "$input<br>";
    echo "[";
    ...
    */
    ...
    if (!ash && !sep) {
        ...
        for (j = 0; j < count(list); j++) {
            ...
            if (substr(input, 0, prefixLength) == prefix) {
                ...
                result = RedoLanguage(remainder, type, allLanguagesBitmap, languageRules, rules, finalRules1, finalRules2, "", concat) + "-" + RedoLanguage(remainder, type, allLanguagesBitmap, languageRules, rules, finalRules1, finalRules2, "", concat);
                return result;
            } else {
                if (substr(input, 0, 2) == "d'") {
                    // check for d'
    ...
    if (sep) {
        ...
        for (i = 0; i < count(words); i++) {
            ...
            // create array of each part between apostrophes
    ...
    if (debug) {
        std::cout << "applying language rules from (" + fileName + ") to <b>" + input + "</b> using languages " + languageArg + "<br><br>";
        std::cout << "char codes =";
        for (i = 0; i < strlen(input); i++) {
            if (ord(input[i]) < 128 + 64) {
                // 1-byte character
                std::cout << " [#" + dechex(ord(input[i])) + "]" + input[i];
            } else {
                if (ord(input[i]) < 128 + 64 + 32) {
                    // 2-byte character
                    std::cout << " [#" + dechex(ord(input[i])) + dechex(ord(input[i + 1])) + "]" + substr(input, i, 2);
                    i++;
                } else {
                    if (ord(input[i]) < 128 + 64 + 32 + 16) {
                        // 3-byte character
                        std::cout << " [#" + dechex(ord(input[i])) + dechex(ord(input[i + 1])) + dechex(ord(input[i + 2])) + "]" + substr(input, i, 3);
                        i += 2;
                    } else {
                        if (ord(input[i]) < 128 + 64 + 32 + 16 + 8) {
                            // 4-byte character
                            std::cout << " [#" + dechex(ord(input[i])) + dechex(ord(input[i + 1])) + dechex(ord(input[i + 2])) + dechex(ord(input[i + 3])) + "]" + substr(input, i, 4);
                            i += 3;
                        } else {
                            if (ord(input[i]) < 128 + 64 + 32 + 16 + 8) {
                                // 5-byte character
                                std::cout << " [#" + dechex(ord(input[i])) + dechex(ord(input[i + 1])) + dechex(ord(input[i + 2])) + dechex(ord(input[i + 3])) + dechex(ord(input[i + 4])) + "]" + substr(input, i, 5);
                                i += 4;
                            } else {
                                if (ord(input[i]) < 128 + 64 + 32 + 16 + 8 + 4) {
                                    // 6-byte character
                                    std::cout << " [#" + dechex(ord(input[i])) + dechex(ord(input[i + 1])) + dechex(ord(input[i + 2])) + dechex(ord(input[i + 3])) + dechex(ord(input[i + 4])) + dechex(ord(input[i + 5])) + "]" + substr(input, i, 5);
                                    i += 5;
                                }
                            }
                        }
    ...
    for (i = 0; i < inputLength;) {
        ...
        for (r = 0; r < count(rules); r++) {
            ...
            if (candidate == /* ORIG: === */ false) {
                //if ($debug) echo "rejecting rule #$r because of incompatible attributes<br>";
                if (debug) {
                    std::cout << "rejecting rule #" + r + " because of incompatible attributes<br>" + "&nbsp;&nbsp;&nbsp;pattern=" + pattern + "<br>" + "&nbsp;&nbsp;&nbsp;lcontext=" + lcontext + "<br>" + "&nbsp;&nbsp;&nbsp;rcontext=" + rcontext + "<br>" + "&nbsp;&nbsp;&nbsp;subst=" + rule[phoneticPos] + "<br>" + "&nbsp;&nbsp;&nbsp;result=" + phonetic + "<br><br>";
                }
                continue;
            }
            phonetic = candidate;
            if (debug) {
                std::cout << "applying rule #" + r + "<br>" + "&nbsp;&nbsp;&nbsp;pattern=" + pattern + "<br>" + "&nbsp;&nbsp;&nbsp;lcontext=" + lcontext + "<br>" + "&nbsp;&nbsp;&nbsp;rcontext=" + rcontext + "<br>" + "&nbsp;&nbsp;&nbsp;subst=" + rule[phoneticPos] + "<br>" + "&nbsp;&nbsp;&nbsp;result=" + phonetic + "<br><br>";
            }
            found = true;
            break;
        }
        if (!found) {
            ...
            if (debug) {
                std::cout << "<b>not found</b>: " + substr(input, i, 1) + "<br><br>";
            }
            patternLength = 1;
        }
        i += patternLength;
    ...
    if (debug) {
        std::cout << "after language rules: <b>" + phonetic + "</b><br><br>";
    }
    ...
    phonetic = ApplyFinalRules(phonetic, finalRules1, languageArg, false, debug);
    // apply common rules
    phonetic = ApplyFinalRules(phonetic, finalRules2, languageArg, true, debug);
...
php_type ApplyFinalRules(php_type phonetic, php_type finalRules, php_type languageArg, php_type strip, php_type debug)
{
    ...
    for (k = 0; k < count(phoneticArray); k++) {
        ...
        if (debug) {
            std::cout << "<br>applying final rules from (" + fileName + ") to " + phonetic + "<br>";
        }
        phonetic2 = "";
        /*
              $attribute = "";
        ...
        */
        ...
        for (i = 0; i < strlen(phonetic);) {
            ...
            for (r = 0; r < count(finalRules); r++) {
                ...
                if (patternLength > strlen(phoneticx) - i || substr(phoneticx, i, patternLength) != pattern) {
                    // no match
                    continue;
                }
                // check that right context is satisfied
                ...
                if (candidate == /* ORIG: === */ false) {
                    if (debug) {
                        std::cout << "rejecting rule #" + r + " because of incompatible attributes<br>";
                    }
                    continue;
                }
                phonetic2 = candidate;
                if (debug) {
                    std::cout << "after applying final rule #" + r + " to phonetic item #" + k + " at position " + i + ": <b>" + phonetic2 + "</b> pattern=" + pattern + " lcontext=" + lcontext + " rcontext=" + rcontext + " subst=" + rule[phoneticPos] + "<br>";
                }
                found = true;
                break;
            }
            if (!found) {
                ...
                if (debug) {
                    std::cout << "no rules match for phonetic item " + k + " at position " + i + ": <b>" + phonetic2 + "</b><br>";
                }
                patternLength = 1;
            }
            i += patternLength;
...
php_type RemoveDuplicateAlternates(php_type phonetic)
{
    ...
    for (i = 0; i < count(altArray); i++) {
        alt = altArray[i];
        if (strpos(result, "|" + alt + "|") == /* ORIG: === */ false) {
            result += "" + alt + "|";
            altcount++;
        }
    }
...
php_type NormalizeLanguageAttributes(php_type text, php_type strip)
{
    ...
    while ((bracketStart = strpos(text, "[")) != /* ORIG: !== */ false) {
        bracketEnd = strpos(text, "]", bracketStart);
        if (bracketEnd == /* ORIG: === */ false) {
            std::cout << "fatal error: no closing square bracket: text=(" + text + ") strip=(" + strip + ")<br>";
            exit;
        }
...
php_type ApplyRuleIfCompatible(php_type phonetic, php_type target, php_type languageArg)
{
    ...
    if (strpos(candidate, "[") == /* ORIG: === */ false) {
